...
- **v2.0.124**：修复 v2.0.123 实测严重 bug（用户 iOS 27 上长按文字弹系统默认菜单、自定义菜单项全部消失）——根因：iOS 26 起弃用 `textView(_:editMenuForTextIn:)`，改用 `textView(_:editMenuForTextInRanges:)`，旧代码只实现了旧 API，iOS 27 上不调用。修复：Coordinator 补实现 iOS 26+ 新 API（ranges 为 `[NSValue]` 包装 UITextRange，取首个转 UITextRange），新旧 API 共用 `buildMenu(for:in:)` 构建完整菜单（复制/选取文字/引用/分享/大爆炸/重新生成/撤回/删除）；「选取文字」改用 iOS 26 新属性 `selectedRanges: [NSRange]`（弃用 selectedTextRange，UITextRange 换算 NSRange 再赋值），选中手按位置词 + 原生拖动手柄
