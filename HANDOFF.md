...
> ⏳ 待发版补充（**同一包 v3.9.13/458**，2026-09-13 追加，与上面灵动岛一起走一次 CI）：①**⑥ 对话内工具进度卡**——后端 `/api/stream/{taskId}` 新增 `toolNames/lastTool/toolSeq`（后端已翻中文，App 不维护第二份映射表；老后端无此键 → 空数组不崩）；App 侧 `AuthStore.streamPoll` 返回元组加 `[String]`、`StreamClient` 透传（只在变化时写入，且写入放在旧代 generation guard **之后**）、`ChatView` 新增 `ToolStepRow` + `@ViewBuilder toolStepCards`（按 `currentStreamSessionId` 收窄防跨会话残留；切会话与云端路径启动时清空）。验证：真值表 27 项 + `./check_swift.sh` 全绿 + 独立只读审查 4 条全部采纳（跨会话残留、旧代 poll 竞态、抽 `@ViewBuilder` 对冲 CI 的 type-check 超时、中止/报错后改未确认图标不再显示绿勾）。②**⑧ AI 生成物 App 内预览**——后端 `/api/stream/media` 放开 `pdf/md/csv/txt/json/log` + `Content-Disposition` 带文件名（**刻意不放开 .html/.svg**：免鉴权端点暴露能跑脚本的类型 = 自开 XSS 面；也不放开可执行文件与压缩包）；App 侧 `MEDIA:` 按扩展名分流——图片仍走图片块，文档走新 `AIFileCard` + `.quickLookPreview`（点击在 App 内打开 PDF/表格/文本，QuickLook 只吃本地文件故先下载到临时目录），文件名净化去 `/` `:` 防路径穿越。验证：后端端到端 15 项 + 真值表 27 项 + 全仓语法通过。**③ 后端同期改动（已上线，不随发版）**：v3.9.14（recover 磁盘兜底 `done` 不再硬编码 true / 清理不再按 mtime 误删在跑任务 / 记忆损坏留档）8 项端到端全绿、v3.9.15（任务中心卡片显示「第 N 步 + 工具名」，**旧版 App 即受益**）、v3.9.16（`/api/stream/{taskId}` 带工具字段）、v3.9.17（媒体服务扩展类型）。备份链 `stream_api.py.bak-v3914~v3917` 可回滚。**④ 已否决不做**：⑦ 危险操作确认闸门（用户原话「不喜欢整天审批，保持现状」）——App 侧弹窗代码已从 ChatView 精确摘除（与 ⑥ 同文件，未误伤）、Hermes 插件目录已删（且从未 enable 过）、后端 `confirm_api.py` 与 `unified_router.py` 两条路由及 pyc 已彻底清除（带 token 验证 5 个端点全 404，对照 `/api/agent/tasks/active` 200）。**本批共 5 个本地提交：`d1af343`(灵动岛) `bc8a07f`(HANDOFF) `f98721d`(⑥工具卡) `64d57b5`(⑧预览) + 本次 HANDOFF 更新；均未 push，用户「攒着」。**
