...
> 最后更新：2026-09-11
> 最新版本：**v3.6.0/437（2026-09-11 已发版，tag `v3.6.0`，CI run#462 success）生活卡片设置 + 卡顿栈根治**：①**生活卡片设置页**（新 `Features/Settings/LifeCardsSettingsView.swift` 1059 行 + 新 `Core/LifeConfig.swift` 409 行，字段与后端 life_api v2 `normalize_config` 逐字一致，不做二次裁剪）——**双入口**：设置 → 「生活卡片」二级页 + 看板股票卡片长按菜单（删除这张卡片 / 添加股票卡片）；**四组可配**：股票卡片（东财 suggest 搜索添加、长按或列表删除、市场中文名）、资讯源（内置 11 个 presets 一键加、自定义 URL+名称、任意增删）、快递（数据源自由/自定义：`url_template` 支持 `{no}/{carrier}/{key}/{phone}` 占位符、列表/时间/上下文/状态字段路径、自定义请求头、单号列表）、价格监控（regex/json 提取 + 分组 + 币种 + 目标价 + 自定义请求头 + 单条试跑 `/api/life/price/test`）；约定：每次改动**整体 POST `/api/life/config` 立即落盘**、保存后按返回值刷新本地（presets 同步）、失败红字不静默；上限（股票/资讯/快递/价格 ≤20）由后端截断。②**卡顿看门狗 v3.6.1 重写**（`Core/DiagnosticsHangWatchdog.swift`）：修 v3.6.0 两个实测缺陷——**栈恒定假阳性**（原只在 RunLoop 恢复点取栈，6 条上报跨 3.5.1/3.5.2 两包、耗时 404ms~118685ms 差别巨大却堆栈逐字节相同 = 拿到的是上报链自己的栈）→ 改为**独立监控线程**：繁忙超阈值即 `thread_suspend` 主线程 + `thread_get_state` 取 pc/fp/lr + 手工 FP 链抓**真实栈**（连采 3 次，地址缓冲挂起前预分配、挂起窗口内零堆分配、只读落在主线程栈区间内的地址防野指针）；**后台时长误报**（退后台/被挂起期间 RunLoop 不跑，恢复后整段挂起被算成「连续繁忙」= 118685ms/51136ms 这类值）→ 加**前台门控**（仅 `applicationState == .active` 的繁忙段计入，退后台即清零起点）+ 250ms 前台心跳、5s 去抖。局限如实说明：Release 包无本地符号，上报是「镜像+偏移」（配 dSYM 可 atos 还原）；完全死锁（主线程永不回 RunLoop）仍无法上报，只保证不误报。③**同版本后端接线（已完成并实测）**：`life_api.py` v2（944 行）部署到 NAS 后端目录（md5 `87454bfbbef64e7fbc052e93bab3bb9e` 与本地源码一致）、`nginx server.d/qingliao_http.conf` + `webui_443.conf` 各加 `/api/life` location、`stream_api` `ALLOWED_RELAY` 加 `"/api/life"`（第 2222 行）；逐层探测：16668 `config=200`、公网 443 `config=200` 且 `cards` 返回真实行情（工业富联/宁德时代）、无 token 401、容器日志 `[router] /api/life -> life_api.LifeHandler: ok`；4 端点在线（`config` GET|POST / `cards` / `stock/search` / `price/test`，中文搜索已实测返回结果）。IPA 已校验 **3.6.0/437**，md5 `3b47c9f0aba2642af31951d1acb76ab4`，大小 2562318，已转存 NAS `轻聊app/qingliao-3.6.0-unsigned.ipa`；本机预检 `check_swift.sh` 5 步全绿 + `LifeConfig` 55 项自测全过（首轮 CI 挂在 arm64 `#if` 分支内两处类型错误：`thread_get_state` flavor 须 `thread_state_flavor_t(Int32)`、`pthread_get_stackaddr_np` 非可选不可条件绑定——该分支 Linux 侧不编译，只能 CI 暴露）
